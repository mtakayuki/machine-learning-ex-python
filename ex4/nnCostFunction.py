...
def nnCostFunction(nn_params, input_layer_size, hidden_layer_size,
    ...
    The parameters for the neural network are "unrolled"
    into the vector nn_params and need to be converted back
    ...
    Y = np.eye(num_labels)[:, y-1].reshape(num_labels, m)

    J = 1 / m * (-Y * np.log(a3) - (1 - Y) * np.log(1 - a3)).sum()
    J += lambda_ / (2 * m) * ((Theta1[:, 1:]**2).sum() + (Theta2[:, 1:]**2).sum())

    delta_3 = a3 - Y
    delta_2 = Theta2.T.dot(delta_3) * sigmoidGradient(np.vstack((np.ones((1, m)), z2)))
    delta_2 = delta_2[1:, :]
    Theta1_grad = delta_2.dot(a1.T) / m
    Theta1_grad += lambda_ / m * np.hstack((np.zeros((Theta1.shape[0], 1)), Theta1[:, 1:]))
    Theta2_grad = delta_3.dot(a2.T) / m
